...
class TransformerPlayer(Player):
   ...
   def _build_scoring_prompt(self, fen: str) -> str:
        ...
        return (
            "You are a strong chess player trying to win the game.\n"
            "Given this position in FEN:\n"
            f"{fen}\n\n"
            "Choose the best winning move in UCI. Prefer checkmate, then checks, strong captures, and improving king safety.\n"
            "Best move in UCI:"
        )
 ...
 # Score candidates in batches (faster on GPU)
   ...
   def _pick_best_scored_move(self, board: chess.Board, fen: str, legal: List[str]) -> Optional[str]:
        ...
        adjusted_scores = []
        for move_uci, score in zip(candidates, cand_scores):
        ...
        best_idx = max(range(len(candidates)), key=lambda k: adjusted_scores[k])
   ...
   def _build_generation_prompt(self, fen: str, legal_moves: List[str]) -> str:
        return (
            "You are an extremely powerful chess engine trying to win.\n"
            "Given this chess position in FEN:\n"
            f"{fen}\n\n"
            "Choose exactly ONE move from the following LEGAL moves (UCI):\n"
            f"{', '.join(legal_moves)}\n\n"
            "Prefer checkmate, then checks, valuable captures, and safe king position.\n"

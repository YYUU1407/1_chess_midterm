...
class TransformerPlayer(Player):
    ...
    def __init__(
        ...
        material_weight: float = 0.1,         # FIX: small winning-oriented bias toward better material
    ):
        super().__init__(name)
        ...
        self.material_weight = material_weight  # FIX: keep LM score dominant, add only slight material bias
        ...
        # FIX: ensure pad token exists for stable batching/generation
    ...
    # FIX: simple material evaluation to bias selection slightly more toward winning
    def _material_score_for_side_to_move(self, board: chess.Board) -> float:
        piece_values = {
            chess.PAWN: 1.0,
            chess.KNIGHT: 3.0,
            chess.BISHOP: 3.0,
    ...
    def _candidate_subset(self, board: chess.Board, legal_uci: List[str]) -> List[str]:
        ...
        promos, checks, captures, castles, others = [], [], [], [], []
        for mv in moves:
            # FIX: check directly in same loop by temporarily making/unmaking the move
            board.push(mv)
            ...
            if mv.promotion is not None:
                ...
                checks.append(mv)  # FIX: prioritize checking moves
    ...
    def _pick_best_scored_move(self, board: chess.Board, fen: str, legal: List[str]) -> Optional[str]:
        ...
        # FIX: combine LM score + small material evaluation so move choice is slightly more winning-oriented
        combined_scores: List[float] = []

        for move_uci, lm_score in zip(candidates, cand_scores):
            mv = chess.Move.from_uci(move_uci)
            board.push(mv)

            # After push, board.turn changed to opponent.
            # So negate score to evaluate from the side that made the move.
            material_score = -self._material_score_for_side_to_move(board)

            # FIX: immediate tactical priorities for stronger winning tendency
            bonus = 0.0
            if board.is_checkmate():
                bonus += 1000.0   # winning move, should dominate everything
            elif board.is_check():
                bonus += 0.2      # slight bonus for giving check

            board.pop()

            combined = lm_score + self.material_weight * material_score + bonus
            combined_scores.append(combined)

        best_idx = max(range(len(candidates)), key=lambda k: combined_scores[k])
